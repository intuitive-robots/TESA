...
with open("lib/qanli/preps.txt", "r") as f:
    common_preps = f.read().splitlines()


def is_aux(tok):
    return (
        tok["form"].lower() in AUX or tok["xpostag"] == "MD" or tok["form"] == "there"
    )


def is_verb(tok):
    return (
        tok["xpostag"].startswith("V")
        or tok["upostag"] in ["VERB", "AUX"]
        or tok["xpostag"] == "MD"
    )
...
class Question:
    ...
    def _get_wh(self):
        ...
        if len(whs) == 0:
            ...
            for wh in whs:
                if (
                    len(question) > wh["id"] + 1
                    and is_verb(question[wh["id"] + 1])
                    and "cl" in question[wh["id"] + 1]["deprel"]
                ):
                    continue
    ...
    def _get_wh_pos(self):
        ...
        for i, tok in enumerate(reversed(question[:wh_idx])):
            if tok["xpostag"] in PREPS or (
                wh_lower == "what"
                and i == 0
                and tok["form"] == "do"
                and question[wh_idx - 2]["form"] == "to"
            ):
        ...
        for tok in question[wh_idx + 1 :]:
            if not is_verb(tok) and (wh_idx <= root_idx or tok["id"] in wh_tok_heads):
                ...
                wh_tok_ids.append(tok["id"])
                if (
                    len(question) > tok["id"] + 1
                    and question[tok["id"] + 1]["form"] == "to"
                ):
    ...
    def _dangling_prep(self):
        ...
        for p in prep:
            if p["deprel"] == "case" and any(
                [tok["head"] == p["id"] for tok in question[: self.root["id"]]]
            ):
                return p
            elif p["deprel"] == "case":
                continue
                ...
                p["xpostag"] in PREPS
                and len(question) > p["id"] + 1
                and question[p["id"] + 1]["head"] > p["id"]
            ):
                return p
    ...
    def _wh_is_time(self):
        ...
        question = self.question
        return (
            wh_lower in ["what", "which"]
            and len(question) > wh_idx + 1
    ...
    def _wh_is_compl(self):
        ...
        if (
            (
                (wh_idx > 0 and question[wh_idx - 1]["xpostag"] in PREPS)
            ...
            or (
                wh_lower in SUBJ_WH
                and self.dangling_prep is not None
                and not self.aux_precedes_verb
            )
            or (
                self.wh_is_time
                and (aux is not None or cop is not None)
                and not self.aux_precedes_verb
            )
        ):
            return True
    ...
    def get_answer_pos(self, a):
        ...
        if self.wh_is_happened and question[endidx]["form"] == "to":
            ...
            (cop is not None and wh_idx == root_idx)
            or aux is None
            or (
                self._is_descendant(wh, subj)
                and not (aux is not None and aux["id"] < root_idx - len(self.aux_toks))
            )
            or self.aux_precedes_verb
        ):
            self._answer_pos = startidx
            self.type = "SUBJ"
        elif (a.type.startswith("V") or a.cop is not None) and (
            root["form"] in VERB_DO or (len(comps) > 0 and comps[0]["form"] in VERB_DO)
        ):
    ...
    def swap_aux(self):
        ...
        neworder = (
            [i for i in range(old_pos)]
            + [i + aux_length for i in range(old_pos, new_pos - aux_length)]
    ...
    def is_do_neg(self):
        if (
            self.aux is not None
            and self.aux["form"] in AUX_DO
            and (
                self.question[self.aux["id"] + 1]["form"] == "not"
                or (
                    self.root["id"] > 0
                    and self.question[self.root["id"] - 1]["form"] == "not"
                )
            )
        ):
            return True
    ...
    def insert_answer_default(self, a):
        ...
        if self.type in ["COMPL", "DOBJ", "VERB"]:
            ...
            and not self.aux_precedes_verb
            and not (
                len(question) > cop["id"] + 1
                and question[cop["id"] + 1]["xpostag"] in PREPS
            )
        ):
        ...
        if (
            question[startidx]["xpostag"] in PREPS
            and a.answer[0]["xpostag"] not in PREPS
        ):
            a = add_affix(
                [a],
                [question[startidx]["form"].lower()],
                question[startidx]["xpostag"],
                "left",
            )[0]
            ...
            self.type == "COMPL"
            and (
                a.answer[0]["xpostag"] not in PREPS
                and question[lastword_idx]["xpostag"] not in PREPS
            )
